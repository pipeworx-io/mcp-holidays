...
`tools/list` at `https://gateway.pipeworx.io/holidays/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/get_holidays \
  -H 'Content-Type: application/json' \
  -d '{"country_code":"US","year":2025}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/get_holidays`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
